...
gulp.task('def', async function () {
  ...
  await bundle.write({
    format: 'umd',
    moduleName: 'ChunkedShow',
    dest: 'dist/chunked-show.js',
  });
});
...
gulp.task('min', async function () {
  ...
  await bundle.write({
    format: 'umd',
    moduleName: 'ChunkedShow',
    dest: 'dist/chunked-show.min.js', // equivalent to --output
    // sourceMap: true
  });
});
...
gulp.task('cjs', async function () {
  ...
  await bundle.write({
    format: 'cjs',
    moduleName: 'ChunkedShow',
    dest: 'dist/chunked-show.cjs.js', // equivalent to --output
    // sourceMap: true
  });
});
